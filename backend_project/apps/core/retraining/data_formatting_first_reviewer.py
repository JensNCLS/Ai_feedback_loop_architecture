...
@log_method_call
def format_first_reviewer_training_data_kfold(k=5):
    try:
        processed_count = 0
        class_names_set = set()
        
        base_dir = Path(__file__).parent.parent.parent.parent / "media"
        kfold_dir = base_dir / "kfolds_first_reviewer"
        data_dir = base_dir / "raw_first_reviewer"
        ...
        for i, fold in enumerate(folds):
            ...
            yaml_content = {
                "path": f"/app/media/kfolds_first_reviewer/fold_{i}",  # Path should be adjusted for docker environment
                "train": "images/train",
                "val": "images/val", 
        ...
        return {
            "status": "success", 
            "message": f"Processed {processed_count} first reviewer images into {k} folds for cross-validation using scikit-learn KFold", 
            "count": processed_count,
            "folds": k,
            "class_names": class_names_list,
            "feedback_images": feedback_images
        ...
        logger.error(f"Error in format_first_reviewer_training_data_kfold: {e}")
        return {"status": "failure", "message": str(e), "count": 0}