...
from ...retraining.retraining import retraining_kfold

class Command(BaseCommand):
    help = 'Executes K-fold cross-validation model training using the formatted data'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Starting {k}-fold cross-validation with epochs={epochs}, batch_size={batch_size}, img_size={img_size}")
        
        result = retraining_kfold(
            k=k, 
            epochs=epochs, 
        ...
        if result.get('status') == 'success':
            self.stdout.write(self.style.SUCCESS(f"Successfully completed {result.get('folds_completed')}/{result.get('total_folds')} folds"))
            ...
            self.stdout.write(self.style.ERROR(f"Failed to complete k-fold training: {result.get('message', 'Unknown error')}"))
